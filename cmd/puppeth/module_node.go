...
// nodeDockerfile is the Dockerfile required to run an Ethcc node.
var nodeDockerfile = `
FROM ethcc/client-go:latest
...
RUN \
  echo 'geth --cache 512 init /genesis.json' > geth.sh && \{{if .Unlock}}
	echo 'mkdir -p /root/.ethcc/keystore/ && cp /signer.json /root/.ethcc/keystore/' >> geth.sh && \{{end}}
...
// an Ethcc node (bootnode or miner for now).
var nodeComposefile = `
version: '2'
services:
  {{.Type}}:
    ...
    ports:
      ...
      - "{{.Port}}:{{.Port}}/udp"
    volumes:
      - {{.Datadir}}:/root/.ethcc{{if .Ethashdir}}
...
// deployNode deploys a new Ethcc node container to a remote machine via SSH,
...
func checkNode(client *sshClient, network string, boot bool) (*nodeInfos, error) {
	...
	stats := &nodeInfos{
		genesis:    genesis,
		datadir:    infos.volumes["/root/.ethcc"],
