...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.ethcc", new NodeConfig());
			node.start();
			...
			EthccClient ec = node.getEthccClient();
	...
	// signatures too. Regression test for healthcare-chain issue #14599.
...
func TestAndroid(t *testing.T) {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/ethcc/gethtest", "libs"} {
	...
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.ethcc", "github.com/tonycai/HealthCare-Chain/mobile")
	...
	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "ethcc", "gethtest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
