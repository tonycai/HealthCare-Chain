...
import (
	...
	"github.com/tonycai/HealthCare-Chain/accounts/usbwallet/internal/trezor"
...
// or whether the Ethcc app was not started on it.
...
// Ethcc address located on that path.
func (w *trezorDriver) trezorDerive(derivationPath []uint32) (common.Address, error) {
	address := new(trezor.EthccAddress)
	if _, err := w.trezorExchange(&trezor.EthccGetAddress{AddressN: derivationPath}, address); err != nil {
...
func (w *trezorDriver) trezorSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	response := new(trezor.EthccTxRequest)
	...
	for response.DataLength != nil && int(*response.DataLength) <= len(data) {
		...
		if _, err := w.trezorExchange(&trezor.EthccTxAck{DataChunk: chunk}, response); err != nil {
			return common.Address{}, nil, err
		}
	}
